...
export default function DaveGamePage() {
  ...
  const canvasWidth = isMobile || isFullscreen ? window.innerWidth : 1500;
  const canvasHeight = isMobile || isFullscreen ? window.innerHeight : 900;

  return (
    <main className="flex flex-col items-center justify-center min-h-screen bg-black text-white relative overflow-hidden">
      {mounted && (
        <h1 className="text-2xl mt-4 mb-2 text-center">{t("gamename")}</h1>
      )}
      ...
      <div
        ...
        className="relative"
        style={{
          width: isMobile || isFullscreen ? "100vw" : "auto",
          height: isMobile || isFullscreen ? "100vh" : "auto",
          overflow: "hidden",
          touchAction: "none",
        }}
      >
